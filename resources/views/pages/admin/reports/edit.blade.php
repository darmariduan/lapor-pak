            ...
            <div class="card-body">
                <form action="{{ route('admin.reports.update', $report->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        @if ($report->image)
                            <div class="mt-2">
                                <img src="{{ asset('storage/' . $report->image) }}" alt="Gambar Laporan" width="100">
                            </div>
                        @endif
                        @error('image')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="address" id="address" rows="2" class="form-control @error('address') is-invalid @enderror">{{ old('address', $report->address) }}</textarea>
                        @error('address')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Set initial coordinates from report data or default to center of Indonesia
            const initialLat = latInput.value ? parseFloat(latInput.value) : -6.200000;
            const initialLng = lngInput.value ? parseFloat(lngInput.value) : 106.816666;
